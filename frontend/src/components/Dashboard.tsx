...
export function Dashboard() {
  ...
  async function fetchDivisions() {
    ...
    const { data, error } = await supabase
      ...
      .order('name') as SupabaseResponse<{ id: string; name: string }>;
  ...
  async function fetchRanges() {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        new_id as id,
        name,
        new_division_id as associated_division_id
      `)
      .eq('associated_division_id', selectedDivision)
      .order('name') as SupabaseResponse<{ id: string; name: string; associated_division_id: string }>;
  ...
  async function fetchBeats() {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        new_id as id,
        name,
        new_range_id as associated_range_id,
        new_division_id as associated_division_id
      `)
      .eq('associated_range_id', selectedRange)
      .eq('associated_division_id', selectedDivision)
      .order('name') as SupabaseResponse<{ 
        id: string; 
        name: string; 
        associated_range_id: string;
        associated_division_id: string;
