      ...
      try {
        ...
        if (selectedBeat) {
          ...
          const { data: beatInfo, error: beatInfoError } = await supabase
            ...
            .single() as { data: BeatInfo | null; error: any };
          ...
          try {
            ...
            throw new Error(`Invalid polygon data format: ${e instanceof Error ? e.message : String(e)}`);
          }
          ...
          const { data: rangeData, error: rangeError } = await supabase
            ...
            .single() as { data: RangeData & { division: { name: string } | null } | null; error: any };
          ...
          if (!rangeData) {
            console.error('No range found for beat:', selectedBeat);
            throw new Error('No range found for this beat');
          }
  ...
  useEffect(() => {
    async function fetchObservations() {
      try {
        ...
        if (!observations) {
          console.error('No observations found for beat:', selectedBeat);
          throw new Error('No observations found for this beat');
        }

        // Type guard to ensure observations match ActivityObservation interface
        const isValidObservation = (obs: unknown): obs is ActivityObservation => {
          return (
            typeof obs === 'object' &&
            obs !== null &&
            'activity_report_id' in obs &&
            'latitude' in obs &&
            'longitude' in obs &&
            'activity_date' in obs &&
            'activity_time' in obs &&
            'observation_type' in obs &&
            'user_id' in obs
          );
        };

        const validObservations = observations.filter(isValidObservation).map(obs => {
          const typedObs = obs as unknown as ActivityObservation;
          return {
            ...typedObs,
            activity_date: new Date(typedObs.activity_date),
            created_at: typedObs.created_at ? new Date(typedObs.created_at) : undefined,
            updated_at: typedObs.updated_at ? new Date(typedObs.updated_at) : undefined
